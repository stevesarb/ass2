...
void output_movies(struct Movie* head) {
...
void create_files(struct Movie* node, char* dir_name) {
    int file_descriptor;
    char* file_path = NULL;
    ssize_t nwritten;
    if (node != NULL) {
        // create the name of the file
        file_path = make_file_name(dir_name, node->year);
        ...
        create_files(node->next, dir_name);
