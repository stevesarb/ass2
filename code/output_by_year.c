...
/*
    So... I just realized that I did a bunch of extra word by organizing the movies into distinct years
    I could've just appended movies to the year file. Oh well. 
    I gave each Movie object a same_year_next pointer, and I used the functions below to connect each movie object
    of the same year together
*/
...
// recursive function to iterate through each Movie node that were released in the same year
void write_movies_to_file(int file_descriptor, struct Movie* node) {
    ...
    if (node != NULL) {
        ...
        write_movies_to_file(file_descriptor, node->same_year_next); // recursive call
    }
}